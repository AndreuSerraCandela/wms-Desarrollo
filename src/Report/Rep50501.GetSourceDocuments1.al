...
report 50501 "Get Source Documents1"
{
            ...
            dataitem("Sales Header"; "Sales Header")
            {
                ...
                dataitem("Sales Line"; "Sales Line")
                {
                    ...
                    var
                        ...
                        SalesWarehouseMgt: Codeunit 50991;
                        IsHandled: Boolean;
                    begin
                        IsHandled := false;
            ...
            dataitem("Purchase Header"; "Purchase Header")
            {
                ...
                dataitem("Purchase Line"; "Purchase Line")
                {
                    ...
                    var
                        ...
                        PurchasesWarehouseMgt: Codeunit "Purchases Warehouse Mgt1";
                        IsHandled: Boolean;
                    begin
                        IsHandled := false;
                            ...
                            case RequestType of
                                RequestType::Receive:
                                    if PurchasesWarehouseMgt.CheckIfPurchLine2ReceiptLine("Purchase Line") then begin
                                        ...
                                        if not PurchasesWarehouseMgt.PurchLine2ReceiptLine(WhseReceiptHeader, "Purchase Line", EsCantidadARecibir) then
                                            ErrorOccured := true;
                                        LineCreated := true;
                                ...
                                RequestType::Ship:
                                    if PurchasesWarehouseMgt.CheckIfFromPurchLine2ShptLine("Purchase Line", ReservedFromStock) then begin
                                        ...
                                        if not IsHandled then begin
                                            ...
                                            if not PurchasesWarehouseMgt.FromPurchLine2ShptLine(WhseShptHeader, "Purchase Line", EsCantidadARecibir) then
                                                ErrorOccured := true;
                                            LineCreated := true;
                                        end;
                                    end;
    ...
    var
        ...
        rpo500: Report "Get Source Documents";



        Text000Err: Label 'There are no Warehouse Receipt Lines created.', comment = 'ESP="Codeunit no iniciada correctamente."';
        Text001Msg: Label '%1 %2 has been created.', comment = 'ESP="%1 %2 se ha creado"';
        Text002Msg: Label '%1 Warehouse Receipts have been created.', comment = 'ESP="Se han creado %1 recep. almacén.';
        Text003Err: Label 'There are no Warehouse Shipment Lines created.', comment = 'ESP="No se han creado líneas de envío de almacén."';
        Text004Msg: Label '%1 Warehouse Shipments have been created.', Comment = '%1 Cabecera de envio se ha creado';
        Text005Err: Label 'One or more of the lines on this %1 require special warehouse handling. The %2 for such lines has been set to blank.', comment = 'ESP="Una o más de las líneas de este %1 requieren manipulación de almacén especial. El %2 para dichas líneas se ha establecido en blanco."';
        Text006Err: Label 'This usually happens when warehouse receipt lines have already been created for a purchase order. Or if there were no changes to the purchase order quantities since you last created the warehouse receipt lines.', comment = 'ESP=Esto suele ocurrir cuando ya se han creado líneas de recepción de almacén para un pedido de compra. O si no hubo cambios en las cantidades del pedido de compra desde la última vez que creó las líneas de recepción de almacén.';
        Text007Err: Label 'There are no new warehouse receipt lines to create', comment = 'ESP="No hay ninguna línea de recepción de almacén nueva para crear"';

        Text009Err: Label 'This usually happens when Warehouse Shipment Lines have already been created for a sales %1. Or there were no changes to sales %1 quantities since you last created the warehouse shipment lines.', comment = 'ESP="Esto suele ocurrir cuando ya se han creado líneas de envío de almacén para un pedido de venta. O si no hubo cambios en las cantidades del pedido de venta desde la última vez que creó las líneas de envío de almacén."';
        Text010Err: Label 'There are no new warehouse shipment lines to create', Comment = 'ESP="No hay líneas de envío de almacén nuevas para crear"';
        NoWhseShipmLinesActionCaptionMsg: Label 'Show open lines', comment = 'ESP="Mostrar líneas abiertas"';
    ...
    begin
        ...
        IF EsCantidadAEnviar THEN SalesLine.SETFILTER("Qty. to Ship", '>0'); //EX-SGG-WMS 100919
    ...
    begin
        ...
        if EsCantidadARecibir then PurchLine.SetFilter("Qty. to Receive", '>0');
    ...
    var
        SalesWarehouseMgt: Codeunit "Sales Warehouse Mgt1";
        IsHandled: Boolean;
    begin
        IsHandled := false;
        ...
        exit(SalesWarehouseMgt.FromSalesLine2ShptLine(WhseShptHeader, SalesLine, EsCantidadAEnviar));
    end;
    ...
    procedure ReciboAlmacenCantidadRecibida(lEscantidadRecibida: Boolean)
    begin
        EsCantidadARecibir := lEscantidadRecibida;
    end;
